...
let wordsList = (str, subStr) => {
  let arr = str.replace(/\./g,'').replace(/,/g,'').replace(/!/g,'').replace(/\?/g,'').replace(/;/g,'').replace(/:/g,'').replace(/"/g,'').toLowerCase().split(' ').filter((arrElem) => arrElem.indexOf(subStr) > -1);
  let collection = new Set();
  arr.forEach((arrElem) => collection.add(arrElem));
  return collection;
};
...
getLocalDate = (date, isSeconds, isISO) => {
  let res;
  ...
  res = (isISO && isSeconds)  ? `${yyyy}-${mm}-${dd}, ${hh}:${mM}:${ss}` : (isISO && !isSeconds) ? `${yyyy}-${mm}-${dd}, ${hh}:${mM}` : (!isISO && !isSeconds) ? `${dd}.${mm}.${yyyy}, ${hh}:${mM}` : `${dd}.${mm}.${yyyy}, ${hh}:${mM}:${ss}`; 
  return res;
...
console.log(getWeekDay('1984-10-24')); //среда
console.log(getWeekDay('2015-02-01')); //воскресенье
console.log(getWeekDay('1981-04-04')); //суббота
console.log(getWeekDay('1983-03-19')); //суббота
console.log(getWeekDay('1956-08-23')); //четверг
/*
 * #4
 *
...
console.log(getLocalDay('2019-07-14'));
...
console.log(getDateAgo('2021-01-03', 35));
console.log(getDateAgo('2021-01-03', 5));
 ...
 */
 ...
 let Car = function (engine, model, name, year){
  ...
  let car = new Car(1900, 'Rifter','Peugeot', 2021);
  let car1 = new Car(3000, 'X1','BMW', 2016);
  let car2 = new Car(3500, 'Rogue','Nissan', 2018);
  let car3 = new Car(2500, 'Jetta','VW', 2011);
  let car4 = new Car(2000, 'Lacetti', 'Chevrolet', 2010);
  let car5 = new Car(5000, 'FX50 AWD', 'Infinite', 2019);

console.log(car.info());
console.log(car1.info());
console.log(car2.info());
console.log(car3.info());
console.log(car4.info()); // chevrolet Lacetti, 2010cc, year 2010, used
car4.used = 'new';
console.log(car4.info()); // chevrolet Lacetti, 2019cc, year 2019, new -- год изменен
car4.used = 'used';
console.log(car4.info()); // chevrolet Lacetti, 2019cc, year 2019, new -- изменения не выполняются
console.log(car5.info()); // infinite FX50 AWD, 2019cc, year 2019, new
car5.used = 'used';
console.log(car5.info()); // infinite FX50 AWD, 2019cc, year 2019, new -- изменения не выполняются
